...
- I don't think the shared groceries are getting picked up
"""
...
def process_expenses():
    ...
    plaid = pd.concat(transactions).sort_values("date").reset_index()
    ...
    plaid = pandasql.sqldf(
        TRANSACTIONS_WITHOUT_PAYMENTS_TO_SPLITWISE_QUERY, locals()
    ).drop("index", axis=1)
    ...
    t.add_column("", ["count", "$"])
    t.add_column("old", [len(plaid), round(sum(plaid["amount"]), 2)])
    is_shared_payment = plaid["transaction_id"].isin(
        [
    ...
    log.debug(
        f"""Shared Venmo payments before adjustment\n{plaid.loc[is_shared_payment, 'amount']}"""
    )
    plaid["amount"] = pandasql.sqldf(
        VENMO_INCOME_NET_OF_SPLITWISE_BALANCE_QUERY, locals()
    )
    t.add_column("new", [len(plaid), round(sum(plaid["amount"]), 2)])
    log.debug(
    ...
    log.info(
        "Now that those are cleaned up, we need to layer Splitwise transactions back in."
    )
    t = PrettyTable()
    t.set_style(PLAIN_COLUMNS)
    t.add_column("", ["count", "$"])
    ...
    print(plaid.columns)
    print(split_transactions.columns)
