...
class ToDo:
    ...
    def parse_symbol(self, todo):
        symbol = todo.strip(' ')[0]
        assert symbol != Symbol.TASK, f'Found a {Symbol.TASK} symbol on {todo}. Make sure to migrate all todos!' 
        return symbol
...
def archive_todos(path, archive_path):
    assert not os.path.exists(archive_path), f'Discontinuing weekly refresh because file already exists at target path {archive_path}'
