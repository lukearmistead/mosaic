...
def verbose_query(plaid, splitwise, query):
    ...
    t.add_column("new", [len(transformed_data), round(sum(transformed_data["amount"]), 2)])
    log.info(t)
    return transformed_data

VENMO_PAYMENTS_FROM_ASPIRATION_TO_SPLITWISE = read_query(f"{transform_dir}venmo_payments_from_aspiration_to_splitwise.sql")
TRANSACTIONS_WITHOUT_VENMO_PAYMENTS_FROM_ASPIRATION_TO_SPLITWISE = f"""
    ...
    """
TRANSACTIONS_WITH_VENMO_INCOME_NET_OF_SPLITWISE_BALANCE = read_query(f"{transform_dir}venmo_income_net_of_splitwise_balance.sql")
TRANSACTIONS_WITH_MY_SHARE_OF_GROUP_AMOUNTS = read_query(f"{transform_dir}transactions_with_my_share_of_group_amounts.sql")
...
def transform_transactions():
    ...
    plaid = verbose_query(plaid, splitwise, TRANSACTIONS_WITHOUT_VENMO_PAYMENTS_FROM_ASPIRATION_TO_SPLITWISE)

    log.info(f"For my income from Venmo, removing Splitwise share from the total")
    shared_transaction_examples = ["Je3vNdZXRVU3oaOPP6p5tKz5zyDyneiqRDrrA", "gvX3p6KMaeIgJB9ZZ3aXSmx8JDoxXDi3yqnVN"]
    is_shared_payment = plaid["transaction_id"].isin(shared_transaction_examples) 
    plaid["amount"] = verbose_query(plaid, splitwise, TRANSACTIONS_WITH_VENMO_INCOME_NET_OF_SPLITWISE_BALANCE)
    ...
    shared_expenses = verbose_query(plaid, splitwise, TRANSACTIONS_WITH_MY_SHARE_OF_GROUP_AMOUNTS)
