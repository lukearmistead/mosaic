...
def get_transaction_data(client, access_token, start_date, end_date) -> pd.DataFrame:
    ...
    while running_date > start_date:
        ...
        if running_date == earliest_date:
            break
        df = df.loc[df['date'] > earliest_date,] # Drop the stub
        dfs.append(df)
...
def main(
    ...
    for account in accounts:
        log.info(f'Pulling transactions from {account} account from {start_date} to {end_date}')
        access_token = yaml_lookup(CREDS_PATH, ACCESS_TOKEN_KEY)[account]['access_token']
