...
def convert_all_dates(config, date_keys=['start_date', 'end_date'], steps=['extract', 'transform']):
    for step in steps:
...
def run_etl(config_path=CONFIG_PATH, creds_path=CREDS_PATH):
   ...
   transform_resting_heart_rates(
        extract_fitbit_path=config['extract']['fitbit']['endpoints']['activities/heart']['output_path'],
   ...
   transform_aggregate(
        ...
        extract_strava_path=config['extract']['strava']['output_path'],
        transform_resting_heart_rates_path=config['transform']['resting_heart_rates']['output_path'],
